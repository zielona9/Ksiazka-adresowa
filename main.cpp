...
struct rekord
{
    ...
    void print() const
    {
        cout<<id<< " " << right << setw( 10 )<<imie<< " " << right << setw(20);
...
int pobieranie_danych_z_pliku(fstream &plik,vector <rekord> &osoba)
{
    ...
    else
    {
        ...
        {
            int poczatek=0;
            persona.id=atoi(pobieranie_pojedynczego_slowa_z_linii(poczatek,linia).c_str());
...
void zapisywanie_rekordu_do_pliku( rekord persona)
{
    ...
    if(zapis.tellg()!=0)
        zapis<<endl;
    zapis<<persona.id<<"|"<<persona.imie<<"|"<<persona.nazwisko<<"|"<<persona.nr_tel<<"|"<<persona.email<<"|"<<persona.adres<<"|";
    zapis.close();
}
...
void dodawanie_nowego_rekordu_wektor( rekord &persona, vector <rekord> &osoba)
{
    ...
    persona.id=najwieksze_id(osoba)+1;
    osoba.push_back(persona);
...
int main()
{
    ...
    ile_rekordow=pobieranie_danych_z_pliku(plik,osoba);
    plik.close();
